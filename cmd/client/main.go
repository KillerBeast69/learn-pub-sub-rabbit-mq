...
func main() {
	...
	for {
		...
		case "spam":
			if len(words) != 2 {
				fmt.Println("Please provide 2 arguments: <spam> <int>")
				continue
			}
			...
			for i := 0; i < n; i++ {
				...
				routingKey := routing.GameLogSlug + "." + gamestate.GetUsername()
				...
				if err != nil {
					fmt.Printf("error publishing mallicious log: %v", err)
				}
				fmt.Printf("published %d mallicious logs\n", n)
			}
		case "quit":
			gamelogic.PrintQuit()
			return
